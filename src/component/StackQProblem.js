...
function StackQProblem() {
    const [obj, setobj] = useState([])
    const [error, seterror] = useState("")
    const [loading, setloading] = useState(false);
    useEffect(() => {
        document.title = "Stack-Queue || PrepAvenger"
        getArrayProblems();
    ...
    return (
        <>
            ...
            <div className="problemcontainer text-light my-5">
                <h1>The PrepAvenger's Stack Queue Problems</h1>
                {
                    loading ?
                        obj.map((value) => {
                            return (
